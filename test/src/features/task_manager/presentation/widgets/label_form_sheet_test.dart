import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:innoscripta_task_manager/src/core/enums/view_state.dart';
import 'package:innoscripta_task_manager/src/features/task_manager/domain/entity/label/create_task_label_entity.dart';
import 'package:innoscripta_task_manager/src/features/task_manager/presentation/blocs/label/label_bloc.dart';
import 'package:innoscripta_task_manager/src/features/task_manager/presentation/widgets/label_form_sheet.dart';
import 'package:mocktail/mocktail.dart';

import '../../../../../helpers/helpers.dart';

void main() {
  late MockLabelBloc mockLabelBloc;

  setUpAll(() {
    registerFallbackValue(
      const CreateTaskLabelEntity(id: '', name: '', color: ''),
    );
  });

  setUp(() {
    mockLabelBloc = MockLabelBloc();
    when(() => mockLabelBloc.state).thenReturn(
      const LabelState.initial(viewState: ViewState.success),
    );
  });

  group('LabelFormSheet', () {
    testWidgets('renders edit mode when label is provided', (tester) async {
      //! Arrange
      const tLabel = TestEntities.tLabelEntity;

      //! Act
      await tester.pumpApp(
        BlocProvider<LabelBloc>.value(
          value: mockLabelBloc,
          child: const Scaffold(
            body: LabelFormSheet(label: tLabel),
          ),
        ),
      );

      //! Assert
      expect(find.text('Edit Label'), findsOneWidget);
      expect(find.text(tLabel.name!), findsOneWidget);
      expect(find.byIcon(Icons.delete_outline), findsOneWidget);
    });
  });
}
